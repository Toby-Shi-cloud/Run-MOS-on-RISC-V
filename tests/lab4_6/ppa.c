...
void srv(u_int pvt) {
	...
	syscall_mem_alloc(0, buf, PTE_W | PTE_R);
	for (i = 0; i < (tot >> 1); ++i) {
		u_int who = 0;
...
int main() {
	syscall_mem_alloc(0, (void *)timer, PTE_W | PTE_R | PTE_LIBRARY);
	*timer = 0;
	if (fork_n(2)) {
		sheep();
	}
	...
	syscall_mem_alloc(0, (void *)shm, PTE_W | PTE_R | PTE_LIBRARY);
